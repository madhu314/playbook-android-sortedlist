...
public class DatasetGridAdapter extends RecyclerView.Adapter<DatasetViewHolder> {
  private ArticleDataset dataset;

  @Override public DatasetViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    return DatasetViewHolder.squareWithHalfMatchingWidth(parent);
  }
    ...
    holder.itemView.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View view) {
        ...
        if (context instanceof ActivityBus) {
          ((ActivityBus) context).onBusEvent(ActivityBus.GRID_ITEM_CLICKED, article);
        }
      }
    });
  }
